export default function FiltersSidebar() {
  return (
    <aside className="w-64 shrink-0 hidden lg:flex flex-col gap-8">
      <div>
        <h3 className="text-sm font-bold text-[#170C79] uppercase mb-3">
          Genre
        </h3>

        {["Fiction", "Philosophy", "History", "Science"].map((item) => (
          <label key={item} className="flex items-center gap-3 mb-2">
            <input type="checkbox" className="accent-[#170C79]" />
      ...
      <div>
        ...
        {["All Titles", "Available Now"].map((item) => (
          <label key={item} className="flex items-center gap-3 mb-2">
            <input type="radio" name="availability" className="accent-[#170C79]" />
            <span className="text-gray-600">{item}</span>
          </label>
        ))}
      </div>
